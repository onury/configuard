...
### Added
- **`Configuard.serializeFlat(configList, edits, options?)`**: the inverse of
...
- **Decryption hook**: an optional, synchronous `decrypt` constructor option,
  ...
  Crypto-agnostic and opt-in; a failing hook throws a `ConfiguardError`.
...
- **`ConfiguardError`**: a dedicated error class (exported from the root) thrown
  ...
  `err instanceof ConfiguardError`.

### Changed
- All thrown errors are now `ConfiguardError` instances; the redundant
  `"Configuard: "` message prefix was dropped (the error `name` identifies them).
...
### Added
- `Configuard` class: builds a nested, typed configuration object from a flat
  `IConfigItem[]` list.
...
- Static `Configuard.parseFlat(configList)`: resolves `${...}` value templates
...
- Reference schema in `docs/config.sql`.
