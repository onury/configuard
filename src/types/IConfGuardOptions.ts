...
/**
 * Options for initializing `ConfGuard`.
 *
 */
export interface IConfGuardOptions {
  /**
   * Whether to enable debug logs. If set to `true`,
   * `ConfGuard` will log detailed information about
   ...
   * object. `ConfGuard` is crypto-agnostic: you supply how to decrypt (your
   * KMS, a symmetric key via Node's `crypto`, etc.).
   *
   * Decryption is **opt-in**: without this hook, `encrypt: true` values are
   * used as-is. If the hook throws, a `ConfGuardError` is thrown. The function
   * must be **synchronous**.
   *
