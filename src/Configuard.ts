...
// own modules
import { ConfiguardError } from './ConfiguardError.js';
...
// `ConfiguardError` class instead (its `name` identifies them).
const MSG_PREFIX = 'Configuard: ';
...
// `"any"` type as auto-detect. Fails loud: throws a `ConfiguardError` if the
...
function parseValue(value: string, type: string, key: string): unknown {
  ...
  try {
    ...
    throw new ConfiguardError(`Failed to parse value "${value}" of key "${key}".`, {
      cause: error,
      key
    });
  }
  if (typeof parsed === 'number' && Number.isNaN(parsed)) {
    throw new ConfiguardError(`Value "${value}" of key "${key}" is not a valid ${type}.`, { key });
  }
  return parsed;
}
...
// validation. Throws a `ConfiguardError` on an invalid value.
...
export class Configuard {
  ...
  /**
   * Initializes a new instance of `Configuard` with the given parameters.
   *
   ...
   * @param [options] Options for initializing `Configuard`. Set
   ...
   *
   * @throws {ConfiguardError} If `accessor` is `"all"`, or `"application"`
   * without an `appLevel`.
   * @throws {ConfiguardError} If the raw config is corrupt: an item with an
   * unparseable `value` (or a `value` that resolves to `NaN`), a missing or
   * circular `${...}` template reference, or a malformed item (invalid
   * `accessor`/`listType`/`type`, or a non-string `key`). `Configuard` fails
   * loud — it never silently skips a corrupt item.
   * @throws {ConfiguardError} If a provided `decrypt` hook throws.
   */
  constructor(
    rawConfigList: IConfigItem[],
    ...
    if (this.accessor === AccessorType.APPLICATION && !this.appLevel) {
      throw new ConfiguardError('Invalid appLevel for application accessor.');
    }
  ...
  // a hook error becomes a `ConfiguardError`.
  private decryptItem(item: IConfigItem): void {
    ...
    try {
      ...
      throw new ConfiguardError(`Failed to decrypt value of key "${item.key}".`, {
        cause: error,
        key: item.key
      });
    }
  ...
  private processTemplate(item: IConfigItem, seen: Set<string> = new Set()): unknown {
    if (!Configuard.isConfigItem(item)) {
    ...
    if (seen.has(item.key)) {
      throw new ConfiguardError(`Circular template reference detected at: "${item.key}"`, {
        key: item.key
      });
    }
    seen.add(item.key);
    ...
    const processedValue = (item.value as string).replace(
      ...
      (match: string, templateKey: string) => {
        ...
        throw new ConfiguardError(`Referenced value for template is missing: "${match}"`, {
          key: item.key
        });
      }
    );
  ...
  private isItemAllowed(item: IConfigItem): boolean {
    ...
    if (item.accessor === AccessorType.APPLICATION && !item.appAccess) {
      throw new ConfiguardError(
        `Invalid appAccess for application accessor (key: "${item.key}").`,
        {
          key: item.key
        }
      );
    ...
    if (isArray(rawList)) {
      rawList.forEach((item: IConfigItem) => {
        ...
        if (typeof item.key !== 'string') {
          throw new ConfiguardError('Corrupt config item: "key" must be a string.');
        }
      ...
      if (isStrSet(item.options as string)) {
        ...
        if (ref) {
          ...
          if (!(name in optionLists)) {
            throw new ConfiguardError(`Referenced option list is missing: "@${name}"`, {
              key: item.key
            });
          }
      ...
      if (options) {
        ...
        for (const v of values) {
          if (!allowed.includes(v)) {
            throw new ConfiguardError(`Value "${v}" of "${item.key}" is not in its option list.`, {
              key: item.key
            });
          }
        }
  ...
  static serializeFlat(
    ...
    const resolveOptions = (it: IConfigItem): string[] | null => {
      ...
      const name = ref[1];
      if (!(name in optionLists)) {
        throw new ConfiguardError(`Referenced option list is missing: "@${name}"`, { key: it.key });
      }
    ...
    const serializeItemValue = (it: IConfigItem): string | null => {
      ...
      if (opts) {
        for (const v of values) {
          if (!opts.includes(v)) {
            throw new ConfiguardError(`Value "${v}" of "${it.key}" is not in its option list.`, {
              key: it.key
            });
          }
        }
    ...
    for (const [key, edit] of Object.entries(edits)) {
      ...
      if (!original) {
        throw new ConfiguardError(`No config item found for key "${key}".`, { key });
      }
      ...
      if ('value' in edit) {
        if (original.editable === false && edit.value !== original.value) {
          throw new ConfiguardError(`Config item "${key}" is not editable.`, { key });
        }
        ...
        if (merged.encrypt && encrypt && isStrSet(serialized as string)) {
          try {
            ...
            throw new ConfiguardError(`Failed to encrypt value of key "${key}".`, {
              cause: error,
              key
            });
          }
