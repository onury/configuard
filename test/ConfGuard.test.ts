...
import { ConfGuard, ConfGuardError } from '../src/index.js';
...
// Captures and returns the ConfGuardError thrown by `fn` (fails if none).
const catchError = (fn: () => unknown): ConfGuardError => {
  try {
    fn();
  } catch (e) {
    return e as ConfGuardError;
  }
  throw new Error('Expected the function to throw, but it did not.');
};

describe('ConfGuard (built from sys-config.json)', () => {
  // Built inside a hook (not at collection time) so Stryker treats these as
  // runtime — not static — mutants.
  let config: ConfGuard;
  beforeEach(() => {
    config = new ConfGuard(rawList, { accessor: AccessorType.SYSTEM }, { debugLogs: false });
  });
...
describe('ConfGuard list types', () => {
  let config: ConfGuard;
  beforeEach(() => {
    config = new ConfGuard(
      [
        item({
          key: 'arr.nums',
...
describe('ConfGuard public API', () => {
  let config: ConfGuard;
  beforeEach(() => {
    config = new ConfGuard([item({ key: 'a.b', type: ValueType.INTEGER, value: '7' })], {
      accessor: AccessorType.SYSTEM
    });
  });
...
describe('ConfGuard accessor access control', () => {
  it('system accessor includes only system/all items', () => {
    const config = new ConfGuard(
      [
  ...
  it('application accessor filters items by bitwise appAccess', () => {
    ...
    const config = new ConfGuard(appList, { accessor: AccessorType.APPLICATION, appAccess: 0b010 });
  ...
  it('throws when an application item is missing its own appAccess', () => {
    const config = () =>
      new ConfGuard([{ ...item({ key: 'a', value: 'x' }), accessor: AccessorType.APPLICATION }], {
        accessor: AccessorType.APPLICATION,
        appAccess: 0b001
      });
  ...
  it('throws when accessor is "all"', () => {
    expect(() => new ConfGuard([], { accessor: AccessorType.ALL })).toThrow(/Cannot set accessor/);
  });
  ...
  it('defaults to "application" and throws (with prefix) when no accessor info is given', () => {
    ...
    expect(() => new ConfGuard([])).toThrow(/Invalid appLevel/);
  });

  it('throws when application accessor has no appLevel', () => {
    expect(() => new ConfGuard([], { accessor: AccessorType.APPLICATION })).toThrow(
      /Invalid appLevel/
    );
  });
});

describe('ConfGuard.isConfigItem', () => {
  it('accepts a well-formed config item', () => {
    expect(ConfGuard.isConfigItem(item({ key: 'x' }))).toEqual(true);
  });

  it('rejects items with an invalid type/listType/accessor', () => {
    expect(
      ConfGuard.isConfigItem({ accessor: 'system', key: 'x', type: 'nope', listType: 'none' })
    ).toEqual(false);
  });

  it('rejects non-objects', () => {
    expect(ConfGuard.isConfigItem(5)).toEqual(false);
    expect(ConfGuard.isConfigItem(null)).toEqual(false);
  });
});

describe('ConfGuard.parseFlat', () => {
  let flat: ReturnType<typeof ConfGuard.parseFlat>;
  beforeEach(() => {
    flat = ConfGuard.parseFlat([
  ...
  it('expands an inline (literal csl) options list and validates against it', () => {
    ...
    expect(() => ConfGuard.parseFlat([item({ key: 'x', value: 'z', options: 'a,b,c' })])).toThrow(
  ...
  it('throws on circular value templates', () => {
    expect(() =>
      ConfGuard.parseFlat([item({ key: 'a', value: '${b}' }), item({ key: 'b', value: '${a}' })])
    ).toThrow(/Circular template reference/);
  });

  it('throws on a missing value template reference', () => {
    expect(() => ConfGuard.parseFlat([item({ key: 'a', value: '${nope}' })])).toThrow(
      /Referenced value for template is missing/
    );
  });

  it('returns an empty result for a non-array input', () => {
    expect(ConfGuard.parseFlat(undefined as unknown as IConfigItem[])).toEqual({
      '@': {},
      configList: []
    });
  });
  ...
  it('handles an @-key with an empty value (empty option list)', () => {
    const out = ConfGuard.parseFlat([item({ key: '@Empty', value: '' })]);
  ...
  it('keeps a null value as null', () => {
    const out = ConfGuard.parseFlat([item({ key: 'x', value: null })]);
...
describe('ConfGuard misc branches', () => {
  ...
  it('handles an empty config list', () => {
    const config = new ConfGuard([], { accessor: AccessorType.SYSTEM });
  ...
  it('throws on an item with a non-string key', () => {
    expect(
      () =>
        new ConfGuard([{ ...item({ key: 123 as unknown as string, value: 'x' }) }], {
          accessor: AccessorType.SYSTEM
        })
...
describe('ConfGuardError.key propagation', () => {
  const sys = { accessor: AccessorType.SYSTEM };

  it('sets `key` on a NaN value error', () => {
    const err = catchError(
      () => new ConfGuard([item({ key: 'nan', type: ValueType.NUMBER, value: 'abc' })], sys)
    );
  ...
  it('sets `key` on a circular template error (build)', () => {
    const err = catchError(
      () =>
        new ConfGuard([item({ key: 'a', value: '${b}' }), item({ key: 'b', value: '${a}' })], sys)
    );
    expect(err.key).toEqual('a');
  });

  it('sets `key` on a missing template error (build)', () => {
    const err = catchError(() => new ConfGuard([item({ key: 'orphan', value: '${nope}' })], sys));
  ...
  it('sets `key` on parseFlat circular / missing / option-list errors', () => {
    ...
    expect(
      catchError(() => ConfGuard.parseFlat([item({ key: 'a', value: '${nope}' })])).key
    ).toEqual('a');
    expect(
      catchError(() =>
        ConfGuard.parseFlat([item({ key: 'x', value: 'a', options: '${@Missing}' })])
...
describe('ConfGuard metadata accessors', () => {
  let config: ConfGuard;
  beforeEach(() => {
    config = new ConfGuard(
      [
...
describe('ConfGuard decryption', () => {
  ...
  it('uses the value as-is when no decrypt hook is provided', () => {
    const config = new ConfGuard([item({ key: 'secret', value: 'enc:x', encrypt: true })], sys);
  ...
  it('warns (debugLogs) when an encrypted item has no decrypt hook', () => {
    ...
    try {
      new ConfGuard([item({ key: 'secret', value: 'enc:x', encrypt: true })], sys, {
        debugLogs: true
      });
  ...
  it('skips decryption for empty/null encrypted values', () => {
    const decrypt = vi.fn(strip);
    const config = new ConfGuard([item({ key: 'empty', value: '', encrypt: true })], sys, {
      decrypt
    });
  ...
  it('throws a ConfGuardError when the decrypt hook fails', () => {
    const decrypt = (): string => {
      throw new Error('bad key');
    };
    const err = catchError(
      () =>
        new ConfGuard([item({ key: 'secret', value: 'enc:x', encrypt: true })], sys, { decrypt })
    );
...
describe('ConfGuard lock (immutability)', () => {
  ...
  it('deep-freezes the built object by default (isLocked === true)', () => {
    const config = new ConfGuard(rows, { accessor: AccessorType.SYSTEM });
  ...
  it('leaves the object mutable when lock: false', () => {
    const config = new ConfGuard(rows, { accessor: AccessorType.SYSTEM }, { lock: false });
...
describe('ConfGuard build() edge cases & logging', () => {
  ...
  it('trims surrounding whitespace from an item key', () => {
    const config = new ConfGuard([item({ key: '  spaced.key  ', value: 'v' })], {
      accessor: AccessorType.SYSTEM
    });
  ...
  it('treats a non-array raw config as empty (no throw)', () => {
    const config = new ConfGuard(undefined as unknown as IConfigItem[], {
      accessor: AccessorType.SYSTEM
    });
    expect(config.data).toEqual({});
    expect(config.isLocked).toEqual(true);
  ...
  it('throws when a value resolves to NaN for a numeric type', () => {
    expect(
      () =>
        new ConfGuard([item({ key: 'nan', type: ValueType.NUMBER, value: 'abc' })], {
          accessor: AccessorType.SYSTEM
        })
  ...
  it('throws when a value cannot be parsed to its declared type', () => {
    ...
    for (const [type, value] of cases) {
      expect(
        () => new ConfGuard([item({ key: 'k', type, value })], { accessor: AccessorType.SYSTEM })
      ).toThrow(/Failed to parse value ".*" of key "k"\./);
    }
  });

  it('throws a ConfGuardError that carries the key and original `cause`', () => {
    let caught: ConfGuardError | undefined;
    try {
      new ConfGuard([item({ key: 'k', type: ValueType.JSON, value: '{bad' })], {
        accessor: AccessorType.SYSTEM
      });
    } catch (error) {
      caught = error as ConfGuardError;
    }
    expect(caught).toBeInstanceOf(ConfGuardError);
    expect(caught?.name).toEqual('ConfGuardError');
  ...
  it('throws on a missing template reference', () => {
    expect(
      () =>
        new ConfGuard([item({ key: 'orphan', value: '${nope}' })], {
          accessor: AccessorType.SYSTEM
        })
  ...
  it('throws on circular template references', () => {
    expect(
      () =>
        new ConfGuard([item({ key: 'a', value: '${b}' }), item({ key: 'b', value: '${a}' })], {
          accessor: AccessorType.SYSTEM
        })
...
describe('ConfGuard.serializeFlat', () => {
  ...
  it('returns only changed rows as a diff (default diffOnly)', () => {
    const out = ConfGuard.serializeFlat(list, { name: { value: 'NewCo' } });
  ...
  it('omits a row whose value did not change', () => {
    const out = ConfGuard.serializeFlat(list, { name: { value: 'Acme' } });
    expect(out.updates).toEqual([]);
  });

  it('canonicalizes numbers and booleans, trims strings', () => {
    const out = ConfGuard.serializeFlat(list, {
      port: { value: ' 9090 ' },
      flag: { value: '1' },
      name: { value: '  Spaced  ' }
    });
  ...
  it('throws on an invalid value for the declared type', () => {
    const err = catchError(() => ConfGuard.serializeFlat(list, { port: { value: 'abc' } }));
    expect(err).toBeInstanceOf(ConfGuardError);
    expect(err.key).toEqual('port');
  });

  it('validates against a referenced option list', () => {
    expect(ConfGuard.serializeFlat(list, { color: { value: 'Red' } }).updates[0].value).toEqual(
      'Red'
    );
    expect(
      catchError(() => ConfGuard.serializeFlat(list, { color: { value: 'Pink' } })).key
    ).toEqual('color');
  });

  it('validates against an inline (csl) option list', () => {
    expect(ConfGuard.serializeFlat(list, { size: { value: 'm' } }).updates[0].value).toEqual('m');
    expect(() => ConfGuard.serializeFlat(list, { size: { value: 'xl' } })).toThrow(
      /not in its option list/
    );
  });
  ...
  it('throws when a referenced option list is missing', () => {
    ...
    expect(() => ConfGuard.serializeFlat(bad, { x: { value: 'a' } })).toThrow(
      /option list is missing/
    );
  });

  it('throws when the key is not found', () => {
    const err = catchError(() => ConfGuard.serializeFlat(list, { ghost: { value: 'x' } }));
  ...
  it('throws when changing a non-editable item value', () => {
    const err = catchError(() => ConfGuard.serializeFlat(list, { locked: { value: 'changed' } }));
  ...
  it('allows a metadata-only edit on a non-editable item', () => {
    const out = ConfGuard.serializeFlat(list, { locked: { editable: true } }, { diffOnly: false });
    expect(out.updates).toEqual([
  ...
  it('aggregates requiresReboot across changed rows', () => {
    expect(ConfGuard.serializeFlat(list, { port: { value: '9000' } }).requiresReboot).toEqual(true);
    expect(ConfGuard.serializeFlat(list, { name: { value: 'X' } }).requiresReboot).toEqual(false);
  });

  it('returns full merged rows when diffOnly is false', () => {
    const out = ConfGuard.serializeFlat(list, { name: { value: 'NewCo' } }, { diffOnly: false });
    expect(out.rows).toHaveLength(1);
    expect(out.rows?.[0]).toMatchObject({ key: 'name', id: 1, value: 'NewCo' });
  });

  it('sets update id to null when the source row has no id', () => {
    expect(ConfGuard.serializeFlat(list, { port: { value: '9000' } }).updates[0].id).toEqual(null);
  });

  it('encrypts an edited encrypt:true value via the hook', () => {
    const encrypt = vi.fn((v: string) => `enc:${v}`);
    const out = ConfGuard.serializeFlat(list, { pwd: { value: 'newsecret' } }, { encrypt });
  ...
  it('stores the plaintext when an encrypted item is edited without an encrypt hook', () => {
    const out = ConfGuard.serializeFlat(list, { pwd: { value: 'newsecret' } });
    expect(out.updates[0].value).toEqual('newsecret');
  });

  it('does not call the encrypt hook for an empty encrypted value', () => {
    const encrypt = vi.fn((v: string) => `enc:${v}`);
    const out = ConfGuard.serializeFlat(list, { pwd: { value: '' } }, { encrypt });
    expect(out.updates[0].value).toEqual(null);
    expect(encrypt).not.toHaveBeenCalled();
  });

  it('throws a ConfGuardError when the encrypt hook fails', () => {
    const encrypt = (): string => {
      throw new Error('kms down');
    };
    const err = catchError(() =>
      ConfGuard.serializeFlat(list, { pwd: { value: 'x' } }, { encrypt })
    );
  ...
  it('emits nothing for an empty (no-op) edit', () => {
    expect(ConfGuard.serializeFlat(list, { name: {} }).updates).toEqual([]);
  });

  it('does not throw when a non-editable item is "edited" to the same value', () => {
    expect(ConfGuard.serializeFlat(list, { locked: { value: 'fixed' } }).updates).toEqual([]);
  });
  ...
  it('emits nothing when a metadata edit matches the current value', () => {
    ...
    expect(ConfGuard.serializeFlat(list, { name: { editable: true } }).updates).toEqual([]);
  });

  it('keeps a none-type value verbatim (not split on commas)', () => {
    expect(ConfGuard.serializeFlat(list, { name: { value: 'a,b' } }).updates[0].value).toEqual(
      'a,b'
    );
  });
  ...
  it('serializes an item whose original value was null', () => {
    const out = ConfGuard.serializeFlat(list, { blank: { value: 'now' } });
  ...
  it('handles a non-array config list and empty edits', () => {
    expect(ConfGuard.serializeFlat(undefined as unknown as IConfigItem[], {})).toEqual({
      updates: [],
      requiresReboot: false
    });
  });
